...
class pyIRcam:
    ...
    def getPositions(self):
        ...
        y = [0x00] * 4
        i = 0
        for j in range(1, 11, 3):  # Decode the data coming from the I2C bus
            x[i] = data[j] + ((data[j + 2] & 0x30) << 4)
            y[i] = data[j + 1] + ((data[j + 2] & 0xC0) << 2)
            i += 1
        i = 0
        for j in ('1', '2', '3', '4'):  # Put the x and y positions into the dictionary
            self.positions[j][1] = x[i]
            self.positions[j][0] = y[i]
            i += 1
        if (all(i == 1023 for i in x) and all(
                i == 1023 for i in y)):  # If all objects are 1023, then there is no IR object in front of the sensor
